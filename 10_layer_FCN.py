...
with open('10_layer_FCN_TIMIT.json','w') as f:    # save the model
    f.write(model.to_json()) 
checkpointer = ModelCheckpoint(filepath='10_layer_FCN_TIMIT.hdf5', verbose=1, save_best_only=True, mode='min')  
#pdb.set_trace()
'''
...
print('load model')
MdNamePath='10_layer_FCN_TIMIT' #the model path
...
for path in Test_Noisy_lists: # Ex: /mnt/Nas/Corpus/TMHINT/Testing/Noisy/car_noise_idle_noise_60_mph/b4/1dB/TMHINT_12_10.wav
    ...
    creatdir(os.path.join("10_layer_FCN_TIMIT_enhanced", noise, speaker, dB))
    librosa.output.write_wav(os.path.join("10_layer_FCN_TIMIT_enhanced", noise, speaker, dB, wave_name), enhanced, 16000)
tEnd = time.time()
...
plt.grid(True)
plt.show()
plt.savefig('10_layer_FCN_TIMIT.png', dpi=150)
